import { cn } from "@/lib/utils";
import { Check } from "lucide-react";
import Link from "next/link";

type Step = {
  id: string;
  label: string;
  complete: boolean;
  optional?: boolean;
  href?: string;
};

export function OnboardingProgress({ steps }: { steps: Step[] }) {
  const completedCount = steps.filter((s) => s.complete).length;
  const currentIndex = steps.findIndex((s) => !s.complete);
  const allComplete = currentIndex === -1;

  return (
    <div className="space-y-3 animate-fade-in-up delay-75">
      <div className="flex items-center justify-between text-sm">
        <span className="font-medium text-foreground">Your progress</span>
        <span className="text-muted">
          {completedCount}/{steps.length} complete
        </span>
      </div>

      {/* Mobile: compact stepper */}
      <div className="flex items-center gap-2 md:hidden">
        {steps.map((step, index) => (
          <div key={step.id} className="flex flex-1 items-center gap-2">
            <div
              className={cn(
                "flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-semibold transition-all duration-300",
                step.complete
                  ? "bg-success-surface text-success dark:bg-green-900/30 dark:text-green-400"
                  : index === currentIndex
                    ? "bg-accent-surface text-accent-foreground ring-2 ring-primary/30"
                    : "bg-stone-100 text-muted dark:bg-stone-800"
              )}
            >
              {step.complete ? <Check className="h-4 w-4" /> : index + 1}
            </div>
            {index < steps.length - 1 && (
              <div className={cn("h-0.5 flex-1 rounded transition-colors", step.complete ? "bg-success dark:bg-green-600" : "bg-stone-200 dark:bg-stone-700")} />
            )}
          </div>
        ))}
      </div>
      <p className="text-xs text-muted md:hidden">
        {allComplete
          ? "🎉 All steps complete!"
          : `Next: ${steps[currentIndex]?.label}${steps[currentIndex]?.optional ? " (optional)" : ""}`}
      </p>

      {/* Desktop: full cards */}
      <ol className="hidden gap-3 md:grid md:grid-cols-4">
        {steps.map((step, index) => {
          const cardClasses = cn(
            "block rounded-lg border px-4 py-3 text-sm transition-all duration-200",
            step.complete
              ? "border-green-200 bg-success-surface text-green-900 dark:border-green-800 dark:bg-green-950/30 dark:text-green-300"
              : index === currentIndex
                ? "border-primary/30 bg-accent-surface text-foreground shadow-sm"
                : "border-border bg-card text-foreground",
            step.href && !step.complete && "hover:shadow-sm hover:border-primary/20 cursor-pointer"
          );

          const cardContent = (
            <>
              <div className="font-medium">
                {index + 1}. {step.label}
                {step.optional ? " (optional)" : ""}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {step.complete ? "✓ Complete" : index === currentIndex ? "→ Current step" : "Upcoming"}
              </div>
            </>
          );

          return (
            <li key={step.id}>
              {step.href && !step.complete ? (
                <Link href={step.href} className={cardClasses}>
                  {cardContent}
                </Link>
              ) : (
                <div className={cardClasses}>
                  {cardContent}
                </div>
              )}
            </li>
          );
        })}
      </ol>
    </div>
  );
}
